...
if strcmp(mapping,'s')==1 % add scramble functionality
   randVector = randperm(nCh);
   sameInd = 0;
   for i=1:nCh
      if randVector(i) == i;
          sameInd = sameInd + 1;
   ...
   while find(diff(randVector)==1) | sameInd > 0
    randVector = randperm(nCh);
    sameInd = 0;
    for i=1:nCh
        if randVector(i) == i;
...
for i=1:nCh
  ...
  elseif strcmp(exc,'sine')==1
    ...
    elseif strcmp(mapping,'s')==1 % scramble mapping
      ModC(i,:)=y.*sin(center(randVector(i))*2.0*pi*[0:(nSmp-1)]/Srate);   
    else fprintf('\nERROR! Mapping must be n, i or s\n');
      return;
    end
...
for i=1:nCh
  if sum(abs(ModC(i,:)))>0
    ...
    elseif strcmp(mapping,'s')==1
      band=filter(filterB(randVector(i),:),filterA(randVector(i),:),ModC(i,:));
      % scale component output waveform to have
      % equal rms to input component
      band=band*level(randVector(i))/norm(band,2);
    end
