function [channels envelopes center wave levels] = vocode(exc, mapping, filters, rectify, smooth, nCh, InFile, OutFile)
%
...
% Change by Mate Aller. For efficiency purposes the function can now
% accept data already loaded into matlab as a structure.
if ischar(InFile)
    [x,Srate] = audioread(InFile);
elseif isstruct(InFile) && isfield(InFile,'y') && isfield(InFile,'Fs')
    x = InFile.y;
    Srate = InFile.Fs;
else
    error('InFile must be a valid path name or a structure');
end
nSmp = length(x);
meen=mean(x);
...
for i=1:nCh
  if sum(abs(ModC(i,:)))>0
    if strcmp(mapping,'n')==1
      ...
      band=band*levels(i)/norm(band,2);
    elseif strcmp(mapping,'i')==1
      band=filter(filterB((nCh+1)-i,:),filterA((nCh+1)-i,:),ModC(i,:));
      % scale component output waveform to have
      % equal rms to input component
      band=band*levels((nCh+1)-i)/norm(band,2);
    end
