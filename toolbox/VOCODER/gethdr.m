...
if strcmp(ext,'ils') %---- ILS ---
  ...
  if cnt < HDRSIZE/2
    fprintf('ERROR reading file %s (possibly a zero-bytes file)\n',filename);
    return;
  end
  Srate1 = xx(62); % getsrate(xx);
%------------------------------------------------------------------
...
%------------------------------------------------------------------
elseif strcmp(ext,'w')  % --vowel files (Hillenbrand et al.)
  HDRSIZE=512;   % Header size in bytes
  xx = fread(fp,HDRSIZE/2,'short');
  Srate1 = 16000;
%------------------------------------------------------------------
elseif strcmp(ext,'wav')  % -- MS Windows WAV files or TIMIT files
  xx=fread(fp,1,'int16');
  riff=(setstr([fix(xx/256) rem(xx,256)]));
  xx=fread(fp,1,'int16');
  riff=[riff (setstr([fix(xx/256) rem(xx,256)]))];
%------------------------------------------------------------------
  if strcmp(riff,'IRFF') == 1   % MS windows .wav file on Intel machine
    ...
    HDRSIZE=ftell(fp);
%------------------------------------------------------------------
  elseif strcmp(riff,'RIFF') == 1   % MS windows .wav file but byte swapped
    xx=fread(fp,4,'int8');  % length in bytes of rest of file
    numsa=lswap(xx);
    xx=fread(fp,7,'short');
    chan=fread(fp,1,'short'); chan=bswap(chan);
    xx=fread(fp,4,'int8');  % -- Get the sampling frequency-------
    Srate1=lswap(xx);
    datasec=fread(fp,2,'short'); datasec=bswap(datasec);
    bytesamp=fread(fp,1,'short'); bytesamp=bswap(bytesamp);
    if bytesamp==1, bps=1; filet='char'; end
    bitssamp=fread(fp,1,'short'); bitssamp=bswap(bitssamp);
    xx=fread(fp,4,'char'); 
    if strcmp('data',setstr(xx)') ~=1
      setstr(xx);
      fprintf('ERROR! Not a valid wav file. Missing data.\n');
    end
    xx=fread(fp,4,'int8');  % Number of samples in bytes
    nsam=lswap(xx);
    HDRSIZE=-ftell(fp);     % Negative value marks byte swap
%------------------------------------------------------------------
  else   % maybe a SPHERE file
    fseek(fp,0,'bof');
    nist=fscanf(fp,'%s',1);
    if strcmp(nist(1:4),'NIST') ==0
      error('Error reading voice file, not a MS Windows WAV nor TIMIT ..');
      return;
    end
    nn=fscanf(fp,'%d',1);
    for i=1:5
...
%------------------------------------------------------------------
elseif strcmp(ext,'raw')  % --Raw files, no header
  HDRSIZE=0;
  Srate1=10000;
%------------------------------------------------------------------
elseif strcmp(ext,'xtt')  % - Ascii file
  HDRSIZE=0;
  Srate1=10000;
  filet='ascii';
%------------------------------------------------------------------
elseif strcmp(ext,'s') %---- ILS ---
  HDRSIZE=12;   % Header size in bytes
  [xx, cnt] = fread(fp,HDRSIZE/2,'short');
  if cnt < HDRSIZE/2
    fprintf('ERROR reading file %s (possibly a zero-bytes file)\n',filename);
    return;
