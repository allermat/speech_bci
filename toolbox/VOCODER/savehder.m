function savehder(fpin,fpout,ext,nsamples,HDRSIZE)
...
elseif strcmp(ext,'wav') % -- WAV files
  ...
  if HDRSIZE>=0      % normal wav file
    ...
    nsam=2*nsamples;
  fwrite(fpout,nsam,'int32');
  else               % We need byte swapping: just write header of voice file
    xx=fread(fpin,-HDRSIZE,'int8');
    fwrite(fpout,xx,'int8');
  end

else
  error('Unknown file extension. Could not save file..');
end
